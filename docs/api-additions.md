...
功能：申请行家浏览页的申请条件列表需要由后台返回逐项状态。左侧圆圈状态由后台字段控制：满足时显示勾选圆，不满足、未提交或未完成时显示空圆。当前前端已接入候选预检接口；接口失败时只展示非用户状态的 `待后台返回` 兜底，不再展示本地样例等级、信用分或组局次数。
...
功能：申请行家内页的限制条件、已有技能领域、字段提示内容、技能标签规则、作品类型和作品数量由后台配置返回；如果后台暂未提供或请求失败，前端仅使用字段结构、校验规则和空表单兜底，不再预填技能标签、简介、作品集或服务报价样例。
...
- 页面进入时调用 `GET /api/app/game-invites/guide-success`，携带 `gameId` / `invitationId` 等 query 参数，接口返回 `hero/successHero`、`timeline`、`party`、`followUps` 后渲染；接口失败或不返回字段时页面 JS 不再回填本地成功文案、积分奖励、玩家、行家、时间线或奖励样例。
...
5. 信用中心已接入 `GET /api/app/profile/credit` 获取信用分卡片、摘要、明细和底部规则说明；信用分标签、分数、状态徽标、底部处罚说明接口不返回时前端隐藏，不再写死默认 100 分和处罚阈值。仍需确认奖励领取、处罚记录详情、分页、空态和是否需要独立提交接口。
...
5. 积分中心页已接入 `GET /api/app/profile/points`，全部展示数据都需由后台提供：可用积分、累计积分、已兑换、过期积分、积分规则、规则限制说明、唯一积分来源、积分比例、计算规则、角色分润示例、角色示例标题、底部积分说明、积分明细记录、收入 / 支出筛选和分页；规则限制说明、角色示例标题和底部说明接口不返回时前端隐藏，不再写死本地业务文案。
...
状态：前端已新增 `GET /api/app/profile/member-center` service/API 封装，`components/member-center` 会按当前会员档位请求后台配置；会员等级、权益、分润比例、雷达文案、适配人群、开通说明、最近开通提示、价格和协议文案不再使用本地静态业务兜底。接口失败时页面保持空配置并提示失败。`立即开通` 支付预下单和协议详情页仍待正式接口确认。
...
状态：前端已新增上述人脉雷达候选接口封装，并在 `components/member-radar-flow` 中接入概览、适配信息、发起匹配、匹配结果和关注接口；本地写死的陆毅 / 胡芳 / 陈总等人物、上海公司地址和固定 `10` 位结果样例已清空。接口失败或未返回结果时保持空态，不再用本地 storage 生成业务结果。
...
功能：技能配置页当前为深色参考图样式的静态走查；进入页面时前端尝试读取技能配置，点击右上角保存时组装当前身份限制、技能槽位、显性技能、隐形技能、服务案例和解锁提示，通过 `PUT /api/app/profile/system-management/skill-config` 提交到后台接口。当前前端已补充本地响应交互：移除技能确认、修改次数不足提示、编辑技能底部面板、解锁新技能确认和添加技能底部面板；这些操作先按页面本地状态更新并扣减剩余修改次数。正式联调时技能槽位、技能列表、修改次数、AI 解锁来源、案例绑定和可操作状态均应由后台返回。
...
1. 身份与限制：当前身份、最多可配置技能数、每月可修改次数、已用次数、剩余次数、重置日期 `resetDate`、超限后的提示文案和是否允许保存。
...
状态：前端已接 `GET /api/app/profile/system-management/skill-config`、`PUT /api/app/profile/system-management/skill-config` 和 `GET /api/app/profile/system-management/skill-config/cases/{caseId}`；页面内技能、可添加候选、服务案例样例和固定重置日期已清空，接口失败时不再回填本地业务案例。正式接口字段、操作权限、修改次数扣减、重置日期、失败码、添加 / 编辑 / 移除 / 解锁接口、候选技能来源和案例绑定流程仍待后端和产品确认。
...
模块：我的 / 系统管理 / 信用中心 / 信用申诉

页面：`pages/profile/system-management/credit-appeal/index`

功能：信用申诉当前为静态走查页；页面展示申诉原因、关联处罚记录、详细说明、证明材料上传、处理时效和提交申诉按钮。正式联调时需要从信用中心或信用明细进入本页，并携带待申诉的信用记录；申诉原因、关联记录、上传材料、提交结果和后续处理状态均应由后台接口支持。

候选接口：

```text
GET /api/app/profile/credit/appeal/options
GET /api/app/profile/credit/records/{recordId}
POST /api/app/profile/credit/appeals/uploads
POST /api/app/profile/credit/appeals
GET /api/app/profile/credit/appeals/{appealId}
```

需要后台返回 / 确认：

1. 申诉原因：原因 key、展示文案、默认选中项、是否允许后台配置和排序；原因列表应由后台返回，后台可增加 / 删除 / 调整展示顺序，前端只按返回列表展示。
2. 关联记录：记录 ID、处罚标题、扣分值、处罚原因、发生时间、是否可申诉、申诉截止时间、记录详情跳转或弹层字段；该记录必须由后台按入口 `recordId` 或可申诉记录列表返回，前端不写死最终记录。
3. 详细说明：最大字数、必填规则、placeholder / 说明文案、敏感词或风控校验失败提示；说明文案应支持后台配置修改。
4. 证明材料：图片数量限制、格式、大小、上传方式、文件 ID 返回结构、删除 / 重传规则；上传材料要求文案应支持后台配置修改。
5. 提交申诉：提交入参、成功后返回的申诉 ID、是否跳转结果页、重复提交和已过期记录的失败码。
...
功能：举报中心当前已接入举报类型配置、举报提交、举报详情、申诉列表、申诉详情、处理记录列表、处理记录详情和撤回申诉接口；首页展示举报类型、被举报人、举报原因、上传证据和温馨提示，详情页展示状态卡、证据材料、处理结果、奖励和时间线。正式联调时仍需要补齐证据上传、对举报发起申诉、处理奖励发放状态、撤回申诉失败态和处理评价能力。
...
1. 举报类型：类型 key、展示文案、排序、是否可用、默认选中项和温馨提示规则文案；温馨提示必须由 `GET /api/app/profile/system-management/reports/options` 返回，前端不再写死处罚金额、奖励比例或信用分规则。
...
8. 举报详情 / 处理详情：举报基本信息、举报内容、证据材料、证据数量或证据区标题、状态卡主副标题、处理结果行、处罚措施、平台奖励、奖励卡、处理说明、时间线和处理人展示字段；详情接口不返回的展示块前端隐藏。
9. 评价处理：评价维度、评分 / 标签 / 文本入参、是否允许重复评价、提交成功后的状态刷新和失败提示。
10. 奖励与信用分：奖励金额、发放状态、信用分增减、处罚描述和规则说明是否完全由后台返回。

状态：前端已接举报类型配置、举报提交、举报详情、处理记录列表、处理记录详情、申诉列表、申诉详情和撤回申诉接口；举报类型、温馨提示、列表、详情状态卡、证据数量、处理结果行、奖励卡、时间线、申诉和处理记录样例已清空。证据上传、处理评价、对举报发起申诉、字段枚举、分页、状态流转、奖励发放、失败码和运行截图复核仍待确认。
...
GET /api/app/profile/settings
PUT /api/app/profile/settings
POST /api/app/profile/settings/cache/clear
POST /api/app/auth/logout
```

需要后台返回 / 确认：

1. 账号安全：支付密码是否已设置、登录密码是否已设置、脱敏手机号、指纹 / 面容支付是否可用与当前开关状态。
2. 通知设置：局消息推送、系统通知、订阅消息、邮件通知、消息免打扰时段和每个开关的可编辑状态。
3. 隐私设置：允许他人查看我的局、允许他人查看我的评价、允许通过手机号找到我、个性化推送的当前状态；隐私政策摘要、第三方共享清单、信息收集清单的跳转路径或 web-view 地址。
4. 通用设置：当前缓存大小、清除缓存成功后的返回值、关于我们版本号和详情页入口。
...
| `successTitle/successDesc/rewardCard/benefits/playOptions` | 评价完成页成功文案、奖励卡、权益和后续参与意向 |

状态：上述页面已接入 `services/game`；再玩一局推荐项、行家成功页三方群信息、引荐记录样例、评价对象、评价标签、评价完成成功文案 / 奖励卡 / 权益 / 意向和 AI 总结样例已清空。分享页活动数据继续使用局详情接口，在线人数文案按用户最新要求暂不清理。`pages/game/referral-record/index.wxml` 当前未传记录 ID 到操作按钮，前端只能按可见记录反查 action，后续建议允许模板补 `data-record-id` 或由后端返回直接跳转路由。
