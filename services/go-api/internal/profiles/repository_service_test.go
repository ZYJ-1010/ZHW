...
func TestRepositoryBackedProfilesRequireRolesAndPersist(t *testing.T) {
	...
	qualification, err := service.UpdateGuideQualification(2, UpdateGuideQualificationRequest{ConditionMet: boolPtr(true), PaymentMet: boolPtr(false)})
	if err != nil {
		t.Fatal(err)
	}
	...
	app, err := service.SubmitRoleApplication(2, SubmitRoleApplicationRequest{RoleCode: "guide", Reason: "want to guide"})
