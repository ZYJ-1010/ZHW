...
func (s *Server) imSocket(w http.ResponseWriter, r *http.Request) {
	...
	_ = client.writeJSON(imSocketOutgoing{Type: "connected", Data: chatRoomPayload(room, userID)})
	client.readLoop()
}
...
func (c *imSocketClient) handleText(payload []byte) {
	...
	c.hub.broadcast(message.RoomID, imSocketOutgoing{Type: "message", RequestID: req.RequestID, Data: message})
}
