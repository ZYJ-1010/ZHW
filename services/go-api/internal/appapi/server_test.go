...
func TestCreateInviteEntryHTTP(t *testing.T) {
	...
	identityService := identity.NewService()
	server := newTestAppServer(authService, identityService)
	server.Configure(config.Config{Wechat: config.WechatConfig{URLLinkBaseURL: "https://wxaurl.example.com/invite"}})
	server.Register(mux)
	...
	if !strings.HasPrefix(first.Data.UrlLink, "https://wxaurl.example.com/invite?") || !strings.Contains(first.Data.UrlLink, "inviteCode="+first.Data.InviteCode) || !strings.Contains(first.Data.UrlLink, "entryType=link") {
		t.Fatalf("expected configured url link with invite params: %s", first.Data.UrlLink)
	}
...
func TestInviteEntryBindsExistingWechatHTTP(t *testing.T) {
	...
	if login.Data.AuthPageMode != "login" || !login.Data.BoundWechat {
	...
	if precheck.Data.AuthPageMode != "login" || !precheck.Data.BoundWechat {
		t.Fatalf("expected bound invite precheck: %s", string(precheckBody))
	}
	postJSON(t, mux, "/api/app/auth/wechat-login", "", `{"code":"entry-other-user","inviteCode":"`+entry.Data.InviteCode+`","entryType":"link"}`, http.StatusForbidden)
}
...
func TestCreditAppealFromCreditLogHTTP(t *testing.T) {
	...
	if strings.Contains(string(appealsAfterBody), "credit_appeal") {
		t.Fatalf("expected handled credit appeal removed from active appeals: %s", string(appealsAfterBody))
	}
}
...
func TestAdminInviteCodeManagementHTTP(t *testing.T) {
	...
	relationsBody := getAdminJSON(t, mux, "/api/admin/invite-relations?inviterUserId=9", adminToken, http.StatusOK)
	...
	if relationsResp.Data.Total != 1 || relationsResp.Data.Items[0].InviteCodeID != created.Data.ID || relationsResp.Data.Items[0].InviterUserID != 9 || relationsResp.Data.Items[0].BindSource != "invite_qrcode" {
...
func TestIdentityFlowHTTP(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/identity/phone/verify", token, `{"realName":"User","idCard":"110101199001011234"}`, http.StatusOK)
...
func TestAdminUsersListSupportsInviteLoggedUsers(t *testing.T) {
	...
	adminToken := adminLoginForTest(t, mux)
	body := getAdminJSON(t, mux, "/api/admin/users?keyword=mock_openid_admin-user-list&realnameStatus=pending&status=active", adminToken, http.StatusOK)
	var resp struct {
		Data struct {
			Total int          `json:"total"`
	...
	if got.ID != user.ID || got.OpenID != user.OpenID || got.RealnameStatus != "pending" || got.Status != "active" {
...
func TestGameApplicationAndManualStartFlow(t *testing.T) {
	...
	gameBody := postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"test game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	var created struct {
		Data struct {
			ID int64 `json:"id"`
		} `json:"data"`
	...
	if len(detailResp.Data.MemberIDs) != 5 || len(detailResp.Data.Progress.Feedbacks) != 1 || detailResp.Data.Progress.Feedbacks[0].Progress != 30 {
	...
	for _, item := range membersResp.Data.Items {
		if item.UserID == 1 && item.Role == "creator" && item.IsCreator {
			seenCreator = true
		}
...
func TestNotificationActionRespondsGameInvitationHTTP(t *testing.T) {
	mux := http.NewServeMux()
	authService := auth.NewService(users.NewStore(), invites.NewStore(), auth.NewTokenStore())
	identityService := identity.NewService()
	newTestAppServer(authService, identityService).Register(mux)
	...
	if len(noticesResp.Data.Sections) == 0 || len(noticesResp.Data.Sections[0].Items) == 0 || len(noticesResp.Data.Sections[0].Items[0].Actions) != 2 {
		t.Fatalf("expected actionable notification section: %s", string(noticesBody))
	}
	...
	actionBody := postJSON(t, mux, "/api/app/notifications/"+strconv.FormatInt(noticesResp.Data.Items[0].ID, 10)+"/actions", inviteeToken, `{"action":"accept","reason":"ok"}`, http.StatusOK)
	...
	if err := json.Unmarshal(actionBody, &actionResp); err != nil {
		t.Fatal(err)
	}
	if !actionResp.Data.Handled || actionResp.Data.Invitation.Status != "accepted" || actionResp.Data.Application.UserID != 2 || actionResp.Data.Application.Status != "pending" {
		t.Fatalf("expected notification action to accept invitation: %s", string(actionBody))
	}
	noticesBody = getJSON(t, mux, "/api/app/notifications", inviteeToken, http.StatusOK)
...
func TestGameInviteAggregatesHTTP(t *testing.T) {
	...
	replayBody := postJSON(t, mux, "/api/app/game-invites/replay", creatorToken, `{"sourceGameId":1,"invitees":[{"id":3,"roleType":"expert"}],"message":"again"}`, http.StatusOK)
	...
	if replayResp.Data.InvitationID == 0 || replayResp.Data.Count != 1 {
...
func TestInvitedMainGuideCanStartAndManageProgressHTTP(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"main guide game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
...
func TestIMFlow(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"IM game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/applications", playerToken, `{"reason":"join"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/applications/1/review", creatorToken, `{"approve":true}`, http.StatusOK)
	...
	if collaborationResp.Data.GameID != 1 || collaborationResp.Data.Progress.Percent == 0 || len(collaborationResp.Data.Progress.Tasks) != 3 || len(collaborationResp.Data.Members) != 5 || len(collaborationResp.Data.Messages) == 0 || collaborationResp.Data.Messages[0].Content != "hello" || collaborationResp.Data.Actions.CanManageMembers || collaborationResp.Data.Actions.EndConfirmRoute == "" {
...
func TestServiceConfirmAndReviewFlow(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"review game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/applications", playerToken, `{"reason":"join"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/applications/1/review", creatorToken, `{"approve":true}`, http.StatusOK)
	...
	if !strings.Contains(playerManageResp.Data.Items[0].ContactExpertRoute, "/pages/im/room/index") || !strings.Contains(playerManageResp.Data.Items[0].PlayerCancelRoute, "/pages/game/player-cancel/index") || !strings.Contains(playerManageResp.Data.Items[0].ReviewRoute, "/pages/game/review/index") {
	...
	if len(managedResp.Data.Items) != 1 || managedResp.Data.Items[0].GameID != 1 || managedResp.Data.Items[0].PlayerID == 0 || managedResp.Data.Items[0].StatusType != "complete" || managedResp.Data.Items[0].ReviewStatus != "pending" || !managedResp.Data.Items[0].CanReview {
		t.Fatalf("expected managed service order to expose review target: %s", string(managedBody))
	}
	if !strings.Contains(managedResp.Data.Items[0].DeliveryRoute, "/pages/game/delivery/index") || !strings.Contains(managedResp.Data.Items[0].ExpertCancelRoute, "/pages/game/expert-cancel/index") || !strings.Contains(managedResp.Data.Items[0].ContactPlayerRoute, "/pages/im/room/index") || !strings.Contains(managedResp.Data.Items[0].ContactGuideRoute, "/pages/im/room/index") || !strings.Contains(managedResp.Data.Items[0].ReviewRoute, "/pages/game/review/index") {
		t.Fatalf("expected managed action routes: %s", string(managedBody))
	}
	if managedResp.Data.Items[0].Actions.FinishDeliveryRoute == "" || managedResp.Data.Items[0].Actions.ExpertCancelRoute == "" || managedResp.Data.Items[0].Actions.ContactPlayerRoute == "" || managedResp.Data.Items[0].Actions.ContactGuideRoute == "" || managedResp.Data.Items[0].Actions.ReviewRoute == "" {
		t.Fatalf("expected managed actions payload: %s", string(managedBody))
	}
	...
	reviewBody := postJSON(t, mux, "/api/app/reviews", creatorToken, `{"gameId":1,"targetUserId":2,"targetRole":"member","score":5,"content":"good","tags":["fun","organized"],"againIntent":"yes"}`, http.StatusOK)
	...
	if len(reviewResp.Data.Review.Tags) != 2 || reviewResp.Data.Review.AgainIntent != "yes" {
...
func TestPlayerCancelRequestRequiresPlayerMemberAndNotifiesCreator(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"player cancel game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/applications", playerToken, `{"reason":"join"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/applications/1/review", creatorToken, `{"approve":true}`, http.StatusOK)

	payload := `{"reasonKey":"need_changed","reasonText":"schedule changed","compensationRate":15,"payAmountText":"¥120"}`
	...
	if resp.Data.Status != "submitted" || resp.Data.CancelRequest.GameID != 1 || resp.Data.CancelRequest.ReasonKey != "need_changed" || resp.Data.CancelRequest.CompensationRate != 15 {
		t.Fatalf("expected submitted cancel request: %s", string(body))
	}
...
func TestExpertCancelRequestRequiresManagerAndNotifiesMembers(t *testing.T) {
	...
	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"expert cancel game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/applications", playerToken, `{"reason":"join"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/applications/1/review", creatorToken, `{"approve":true}`, http.StatusOK)

	payload := `{"reasonKey":"schedule_conflict","reasonText":"unexpected schedule","compensationRate":20,"payAmountText":"¥176"}`
	...
	if resp.Data.Status != "submitted" || resp.Data.CancelRequest.GameID != 1 || resp.Data.CancelRequest.ReasonKey != "schedule_conflict" || resp.Data.CancelRequest.CompensationRate != 20 {
		t.Fatalf("expected submitted expert cancel request: %s", string(body))
	}
...
func TestAdminBatchHandleReports(t *testing.T) {
	...
	completeIdentityForTest(t, mux, userToken)
	postJSON(t, mux, "/api/app/games", userToken, `{"title":"batch report game","gameType":"free","minPlayers":5,"maxPlayers":8}`, http.StatusOK)
	...
	operatorToken := adminLoginForTestAs(t, mux, "operator", "admin123")
	postAdminJSON(t, mux, "/api/admin/reports/batch-handle", operatorToken, `{"reportIds":[1],"action":"handle","adminId":3,"result":"blocked"}`, http.StatusForbidden)

	body := postAdminJSONWithPermission(t, mux, "/api/admin/reports/batch-handle", "report:handle", `{"reportIds":[`+strconv.FormatInt(first.Data.ID, 10)+`,`+strconv.FormatInt(second.Data.ID, 10)+`],"action":"handle","adminId":1,"result":"batch handled"}`, http.StatusOK)
	var resp struct {
		Data struct {
			Success int `json:"success"`
			Failed  int `json:"failed"`
...
func TestAppHomeAndConnectionNetworkPayloadHTTP(t *testing.T) {
	...
	if len(homeResp.Data.FriendGames) == 0 {
		t.Fatalf("expected friend game cards: %s", string(homeBody))
	}
