...
func TestManualStartPersistsBroadcastsAndNotifiesMembers(t *testing.T) {
	mux := http.NewServeMux()
	authService := auth.NewService(users.NewStore(), invites.NewStore(), auth.NewTokenStore())
	identityService := identity.NewService()
	app := newTestAppServer(authService, identityService)
	app.Register(mux)
	server := httptest.NewServer(mux)
	defer server.Close()

	creatorToken := loginForTestWithCode(t, mux, "manual-start-creator")
	completeIdentityForTest(t, mux, creatorToken)
	playerToken := loginForTestWithCode(t, mux, "manual-start-player")
	completeIdentityForTest(t, mux, playerToken)

	postJSON(t, mux, "/api/app/games", creatorToken, `{"title":"开局通知测试","gameType":"free","minPlayers":5,"maxPlayers":8,"startAt":"2030-01-01 10:00","endAt":"2030-01-01 12:00"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/approve-local", creatorToken, `{}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/1/applications", playerToken, `{"reason":"join"}`, http.StatusOK)
	postJSON(t, mux, "/api/app/games/applications/1/review", creatorToken, `{"approve":true}`, http.StatusOK)
	...
	historyBody := getJSON(t, mux, "/api/app/games/1/chat/messages", playerToken, http.StatusOK)
	if !bytes.Contains(historyBody, []byte("《开局通知测试》局开始了")) {
		t.Fatalf("expected manual-start message persisted in history: %s", string(historyBody))
	}

	playerNotices := getJSON(t, mux, "/api/app/notifications", playerToken, http.StatusOK)
	if countNotificationsByType(t, playerNotices, "game_started") != 1 || bytes.Contains(playerNotices, []byte(`"needWechat":true`)) {
		t.Fatalf("expected in-app game_started notification without WeChat task: %s", string(playerNotices))
	}
	messageCenter := getJSON(t, mux, "/api/app/messages/center", playerToken, http.StatusOK)
	if !bytes.Contains(messageCenter, []byte("组局已开局")) {
		t.Fatalf("expected manual-start notification in personal message center: %s", string(messageCenter))
	}
	if !bytes.Contains(messageCenter, []byte(`"routeKey":"im_room"`)) ||
		!bytes.Contains(messageCenter, []byte(`/pages/im/room/index?gameId=1`)) ||
		!bytes.Contains(messageCenter, []byte("最新消息：《开局通知测试》局开始了")) {
		t.Fatalf("expected group IM entry with the start message in personal message center: %s", string(messageCenter))
	}
	...
	if err := json.Unmarshal(playerNotices, &noticeResponse); err != nil {
		t.Fatal(err)
	}
	var gameStartedNotificationID int64
	for _, item := range noticeResponse.Data.Items {
		if item.NotifyType == "game_started" {
			gameStartedNotificationID = item.ID
			break
		}
	}
	if gameStartedNotificationID <= 0 {
		t.Fatalf("expected game_started notification id: %s", string(playerNotices))
	}
	actionBody := postJSON(t, mux, "/api/app/notifications/"+strconv.FormatInt(gameStartedNotificationID, 10)+"/actions", playerToken, `{"action":"detail"}`, http.StatusOK)
	if !bytes.Contains(actionBody, []byte(`"route":"pages/im/room/index?gameId=1"`)) {
		t.Fatalf("expected personal message to open the game IM room: %s", string(actionBody))
	}
}
