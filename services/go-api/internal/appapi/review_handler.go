...
func (s *Server) createReviewRemindNotifications(gameID int64) {
	...
	for _, userID := range s.games.Members(gameID) {
		...
		for _, todo := range todos {
			...
			s.notices.Create(notifications.CreateRequest{
				...
				Content:     "The game has ended. Please complete your review.",
				BizType:     "game",
				BizID:       gameID,
				NeedWechat:  true,
				WechatState: "pending",
...
func (s *Server) submitReview(w http.ResponseWriter, r *http.Request) {
	...
	review, profile, err := s.reviews.Submit(userID, req)
	if err != nil {
		writeReviewError(w, err)
		return
	}
	...
	httpx.OK(w, map[string]interface{}{"review": review, "profile": profile, "credit": credit})
}
