...
func (s *Server) getSystemProfileInfo(w http.ResponseWriter, r *http.Request) {
	...
	httpx.OK(w, s.profiles.SystemManagementConfig(userID, "profile-info", s.defaultSystemProfileInfo(userID)))
}
...
func (s *Server) saveSystemProfileInfo(w http.ResponseWriter, r *http.Request) {
	...
	if personal, ok := objectField(payload, "personalInfo"); ok {
		...
		if name != "" || avatarFileID > 0 {
			...
			nextAvatarFileID := user.AvatarFileID
			if avatarFileID > 0 {
				nextAvatarFileID = avatarFileID
			}
			if _, err := s.auth.UpdateProfile(userID, nextName, user.AvatarURL, nextAvatarFileID); err != nil {
