...
func reviewSelect() string {
	return `select id, game_id, reviewer_user_id, target_user_id, target_role, score, content, coalesce(tags, '[]'::jsonb), again_intent, nps_score, created_at from reviews `
}
...
func scanReview(row interface {
	...
	var npsScore sql.NullInt64
	var tags []byte
	err := row.Scan(&item.ID, &item.GameID, &item.ReviewerUserID, &item.TargetUserID, &item.TargetRole, &item.Score, &content, &tags, &againIntent, &npsScore, &item.CreatedAt)
	item.Content = content.String
	_ = json.Unmarshal(tags, &item.Tags)
	item.AgainIntent = againIntent.String
	if npsScore.Valid {
		value := int(npsScore.Int64)
		item.NPSScore = &value
	}
	return item, err
}
