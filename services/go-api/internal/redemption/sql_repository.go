...
insert into redemption_items (name, description, points_cost, stock, status, created_at, updated_at)
values ($1,$2,$3,$4,$5,$6,$7)
returning id, name, description, points_cost, stock, status, created_at, updated_at
`, item.Name, item.Description, item.PointsCost, item.Stock, item.Status, item.CreatedAt, item.UpdatedAt))
}

func (r *SQLRepository) ListItems(ctx context.Context, includeInactive bool) ([]Item, error) {
	query := `select id, name, description, points_cost, stock, status, created_at, updated_at from redemption_items`
	if !includeInactive {
...
select id, name, description, points_cost, stock, status, created_at, updated_at
...
set name = $2, description = $3, points_cost = $4, stock = $5, status = $6, updated_at = $7
where id = $1
returning id, name, description, points_cost, stock, status, created_at, updated_at
`, item.ID, item.Name, item.Description, item.PointsCost, item.Stock, item.Status, item.UpdatedAt))
}
...
func orderSelect() string {
	return `
select o.id, o.order_no, o.user_id, o.item_id, coalesce(i.name, ''), o.points_cost, o.status,
...
func scanItem(row interface {
	...
	if err := row.Scan(&item.ID, &item.Name, &description, &item.PointsCost, &item.Stock, &item.Status, &item.CreatedAt, &item.UpdatedAt); err != nil {
		return Item{}, err
	}
...
func scanOrder(row interface {
	...
	if err := row.Scan(&order.ID, &order.OrderNo, &order.UserID, &order.ItemID, &order.ItemName, &order.PointsCost, &order.Status, &reviewAdminID, &reviewReason, &order.CreatedAt, &order.UpdatedAt); err != nil {
		return Order{}, err
	}
