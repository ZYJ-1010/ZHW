...
func TestRepositoryStorageKeysRemainUniqueAcrossServiceRestarts(t *testing.T) {
	repo := &fakeFileRepository{files: make(map[int64]File)}
	create := func() File {
		service := NewServiceWithRepository(repo)
		_, file, err := service.CreateUploadToken(2, UploadTokenRequest{
			BizType:  "game_cover",
			FileName: "cover.png",
			MimeType: "image/png",
			Size:     128,
	...
	if first.StorageKey == second.StorageKey {
		t.Fatalf("storage key must survive process-local counter resets: %q", first.StorageKey)
	}
}
