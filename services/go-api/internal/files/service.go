...
func (s *Service) UseCOSPostSigner(signer *COSPostSigner) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.cosSigner = signer
}
...
func (s *Service) CreateUploadToken(userID int64, req UploadTokenRequest) (UploadToken, File, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	storageID := fmt.Sprintf("%d", s.nextID)
	if s.repo != nil {
		storageID = uniqueStorageID(s.nextID)
	}
	...
	file := File{
		...
		Size:        req.Size,
		SHA256:      req.SHA256,
		StorageKey:  fmt.Sprintf("%s/%d/%s-%s", req.BizType, req.ObjectID, storageID, req.FileName),
	...
	uploadURL, formData, headers, err := s.uploadTarget(file.StorageKey, file.ID, policy.maxSize)
	if err != nil {
		return UploadToken{}, File{}, fmt.Errorf("create upload target: %w", err)
	}
	return UploadToken{
		FileID:     file.ID,
		UploadURL:  uploadURL,
...
func uniqueStorageID(fallback int64) string {
	var value [12]byte
	if _, err := rand.Read(value[:]); err == nil {
		return hex.EncodeToString(value[:])
	}
	return fmt.Sprintf("%d-%d", time.Now().UnixNano(), fallback)
}
...
func (s *Service) uploadTarget(storageKey string, fileID int64, maxSize int64) (string, map[string]string, map[string]string, error) {
	if s.cosSigner != nil {
		if s.uploadBaseURL == "" {
			return "", nil, nil, ErrStorageNotConfigured
		}
		formData, err := s.cosSigner.SignForm(storageKey, maxSize)
		if err != nil {
			return "", nil, nil, err
		}
		return s.uploadBaseURL, formData, map[string]string{}, nil
	}
	uploadURL, err := s.uploadURL(storageKey)
	if err != nil {
		return "", nil, nil, err
	}
	return uploadURL, nil, map[string]string{
		"x-zhw-file-id": fmt.Sprintf("%d", fileID),
	}, nil
}
