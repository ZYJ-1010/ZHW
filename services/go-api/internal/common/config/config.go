...
func (c Config) ReadinessReport() ReadinessReport {
	...
	report.Items = append(report.Items,
		...
		readinessItem("storage_secret", production && strings.EqualFold(c.Storage.Provider, "minio"), !strings.EqualFold(c.Storage.Provider, "minio") || !isPlaceholderSecret(c.Storage.MinIOSecretKey), "MINIO_SECRET_KEY is configured when STORAGE_PROVIDER=minio"),
