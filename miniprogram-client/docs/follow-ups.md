...
## 2026-07-11 邀请注册链路处理结果

- `UNDEFINED` 邀请码：已通过邀请上下文归一化、无效字符串过滤和启动页带码跳转修复；二维码 `scene` 会解析出 `inviteCode` 与 `entryType`，预检成功后进入手机号注册页。
- 手机号注册登录：已新增公开接口 `POST /api/app/auth/phone-login`，前端 `loginByPhone()` 不再调用 `loginByWechat()`；新用户校验并绑定邀请码，已有手机号登录原账号。
- 验证码：当前按产品确认临时使用 `000000`，真实短信发送和校验留待后续替换。
- 旧记录说明：下方 2026-07-02 的“暂不修复”内容保留为历史证据，其当前状态以上述处理结果为准。
- 发布状态：生产迁移 `000036_user_phone_auth.sql` 和 Go API 已于 2026-07-11 发布并通过无副作用接口验证。
- 待完成：在微信开发者工具/真机用真实有效邀请码完成一次端到端注册，确认页面跳转、token 保存和后续页面进入结果。
...
## 2026-07-12 交付确认页参考版对齐与接口收口

### 功能模块：组局 / 服务交付确认

- 已修复：行家和玩家根据后端 `completion.role/viewer.role` 显示对方参与人及真实角色标签，不再固定显示玩家资料和“行家”标签。
- 已修复：确认时间线优先读取后端 `state/lineState/action`，联系操作统一使用后端 `primaryContactKey/Text/Prefill`，不再把后台状态按前端序号重算。
- 已修复：接口新增 `deliveryMode`；从通知进入时补齐 `mode`，前端仍以详情接口为最终依据，免费局隐藏交付凭证和上传快捷操作。
- 已修复：已确认用户的确认项自动全部勾选并锁定；接口缺少 `completion.canConfirm=true` 或加载失败时不允许提交。
- 视觉处理：免费局恢复参考工程的蓝色完成卡、白底活动卡、绿色说明区、确认卡和底栏样式；付费局保留当前通用样式和自适应页面壳。
- 图标处理：参考工程的 `contact-person.svg` 与 `shield-service.svg` 和当前有效目录内容已一致，继续使用项目内 SVG，不引入参考工程 PNG 或外链替代。
- 验证结果：`node --check pages/game/delivery/index.js` 通过；`go test ./internal/appapi -run '^$'` 编译通过；交付顺序定向测试通过。
- 待验证：在微信开发者工具外层项目中分别使用免费局行家、免费局玩家、付费局行家、付费局玩家四种身份实测角色卡、时间线联系按钮、底部禁用状态和评价跳转。
