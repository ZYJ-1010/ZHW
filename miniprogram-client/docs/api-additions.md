...
## 2026-07-12 交付确认页模式与确认状态收口

- 页面：`pages/game/delivery/index`。
- 接口：`GET /api/app/games/{gameId}/success-detail` 新增 `deliveryMode=free|paid`，前端以接口字段为准选择免费局或付费局页面配置；通知处理路由同步携带 `mode`。
- 接口：`POST /api/app/games/{gameId}/service-confirm-items` 与 `POST /api/app/games/{gameId}/service-confirm` 统一接收并校验 `confirmItemKeys`，确认结果继续保留原有幂等、角色顺序和评价流转逻辑。
- 状态：前后端已完成；前端语法检查、Go API 编译和交付顺序定向测试通过。

## 2026-07-11 局内协作结束与交付确认

- 页面：`pages/game/collaboration/index`、`pages/game/delivery/index`、`pages/game/review/index`。
- 接口：`POST /api/app/games/{gameId}/completion-request`。
- 后台创建局：按 `gameSource=admin` 识别，所有成员均按玩家处理；结束后直接进入 `pending_review`，向各成员发送结束和评价通知，同时记录后台可追踪的结束事件。
- 玩家创建局：结束后进入 `pending_confirm` 并通知全体成员；`expert` 必须先确认，全部行家确认后才允许 `member/player` 确认；`guide/main_guide` 不参与交付确认。
- 交付接口：继续使用 `POST /api/app/games/{gameId}/service-confirm`；成功进入 `pending_review` 后前端自动进入评价页。
- 状态：前端、Mock 与 Go API 已接入，Go 业务层顺序测试已通过。

## 2026-07-11 邀请注册 / 手机号注册登录

- 模块：邀请注册 / 账号认证
- 页面：`pages/entry/index`、`pages/login/index`
- 接口：`POST /api/app/auth/phone-login`
- 用途：新用户从有效邀请码入口进入后，使用手机号和验证码创建账号并绑定邀请关系；已绑定手机号的用户再次提交时登录原账号，不再转调微信登录。
- 请求：`phone`、`code`、`inviteCode`、`entryType`。新手机号注册必须带有效邀请码，已有手机号账号登录可不带邀请码。
- 临时验证码：按当前产品确认只接受 `000000`；后续接入真实短信时替换服务端验证码校验，不改变前端登录方法和接口路径。
- 数据：`users.mobile_hash` 保存由生产 `JWT_SECRET` 参与计算的 HMAC-SHA256 查重值，接口只返回脱敏手机号，不保存或返回手机号明文。
- 兼容：已有微信用户完成手机号绑定后，可使用该手机号登录同一用户；原微信登录接口和微信账号关系继续保留。
- 状态：前后端已完成；`db/migrations/000036_user_phone_auth.sql` 与 Go API 已于 2026-07-11 发布到生产，生产域名接口验证通过，待微信开发者工具/真机使用真实邀请码完成一次完整注册。
...
- `detailDisplay.primaryAction` 由后台结合 `game.status` 和 `myRelation` 生成，返回 `text`、`disabled`、`action`、`route`、`confirmText`；前端不再写死“立即报名”。
- `detailDisplay.organizer.rating` 只在组局者存在真实收到的评价时返回，同时返回 `ratingCount`；无评价时前端隐藏整个评分区域。
- `GET /api/app/game-applications/received` 支持 `gameId` 与 `status` 联合过滤，从详情页进入时只展示本局申请。
...
# 2026-07-12 生产前后端权威数据收口

- 取消服务详情：新增 `GET /api/app/games/{gameId}/player-cancel-detail`、`GET /api/app/games/{gameId}/expert-cancel-detail`。名称、金额、赔付比例、服务时长、状态和联系对象均由外层 Go 后端计算并返回，前端不再消费路由业务字段。
- 支付预览：新增 `GET /api/app/games/{gameId}/payment-preview`。支付金额、币种、支付状态、分项和规则由后端返回；创建订单接口继续只接收 `gameId`。
- 举报边界：`POST /api/app/reports` 增加局参与者、目标用户、自我举报和活动中重复举报校验。
- 服务确认：`POST /api/app/games/{gameId}/service-confirm` 接收 `confirmItemKeys`，后端校验完整确认项并保存到确认记录；旧客户端未传时按当前后端配置归一，保证发布兼容。
- 状态：生产前端与外层 `services/go-api` 已完成；未修改 Mock 和内层历史后端副本。
