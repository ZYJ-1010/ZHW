  ...
  methods: {
    ...
    initRecorder() {
      ...
      this.recorder.onStart(() => {
        this.setData({ isRecording: true, activePanel: '' })
    ...
    handleVoiceTap() {
      this.handleToolTap({ currentTarget: { dataset: { type: 'voice' } } })
      if (this.data.isRecording) {
        this.stopRecord()
        return
      }
    ...
    startRecord() {
      ...
      wx.authorize({
        scope: 'scope.record',
        success: () => this.recorder.start({ duration: 60000, format: 'mp3' }),
        fail: () => wx.showModal({
          title: '需要录音权限',
    ...
    handleSendOrAttachTap() {
      ...
      this.setData({ activePanel: this.data.activePanel === 'attach' ? '' : 'attach' })
      this.triggerEvent('panelchange', { panel: this.data.activePanel })
    ...
    handleChooseImage() {
      if (wx.chooseMedia) {
        wx.chooseMedia({
          ...
          success: (result) => this.triggerEvent('chooseimage', result)
        })
        return
      }
      wx.chooseImage({
    ...
    handleChooseFile() {
      ...
      wx.chooseMessageFile({
        count: 1,
        type: 'file',
        extension: ['pdf', 'txt', 'zip', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'],
        success: (result) => this.triggerEvent('choosefile', result)
      })
    }
  }
})
