...
async function boot() {
  try {
    ...
    $("#admin-role").textContent = `${adminDisplayValue(state.admin?.username || "admin")} / ${roleSnapshot(state.admin?.roles || [])}`;
    setAPIStatus(true);
    applyNavPermissions();
    ensureAllowedView();
    setActiveNav();
...
async function showUserDetail(id) {
  ...
  panel.innerHTML = `
    <div class="panel-head">
      <div>
        <h2>${escapeHTML(userText(user.id))}详情</h2>
        <p>${escapeHTML(user.nickname || wechatBindingText(user.openId))}</p>
      </div>
...
async function loadInviteCodes(formData) {
  ...
  $("#invite-codes-table").innerHTML = state.inviteCodes.map(inviteCodeRow).join("") || emptyRow(7, "暂无邀请码");
}
...
async function showGameDetail(id) {
  ...
  panel.innerHTML = `
    <div class="panel-head">
      <div>
        <h2>${escapeHTML(data.game.id ? `局 ${data.game.id}` : "组局")}详情</h2>
...
async function loadIdentities() {
  ...
  list.innerHTML = items.map((item) => stackItem({
    ...
    badge: item.status,
    meta: [`手机号：${item.phone || "-"}`, `认证姓名：${item.realname || "-"}`, `最近更新：${formatTime(item.updatedAt || item.createdAt)}`],
    action: `<button class="ghost" data-action="identity-detail" data-user-id="${item.userId}" type="button">详情</button>`,
  })).join("") || emptyBlock("暂无认证记录");
}
...
async function showIdentityDetail(userID) {
  ...
  panel.innerHTML = `
    ...
    <div class="detail-grid">
      ...
      ${detailCell("认证方式", item.faceIdRequestId ? "人脸核身" : "短信验证")}
      ${detailCell("手机号", item.phone || "-")}
      ${detailCell("姓名", item.realname || "-")}
...
async function loadRedemptionItems() {
  ...
  $("#redemption-items-table").innerHTML = state.redemptionItems.map(redemptionItemRow).join("") || emptyRow(8, "暂无兑换商品");
}
...
async function showMemberTeamDetail(teamID) {
  ...
  panel.innerHTML = `
    <div class="panel-head">
      <div>
        <h2>${escapeHTML(team.id || teamID ? `团队 ${team.id || teamID}` : "团队")}详情</h2>
...
async function renderProfiles() {
  ...
  $("#expert-profile-panel").innerHTML = emptyBlock("输入行家用户后查询技能画像");
  $("#guide-profile-panel").innerHTML = emptyBlock("输入领路人用户后查询资源画像");
  if (can("connection:read")) {
...
async function loadReports() {
  ...
  $("#reports-table").innerHTML = state.reports.map(reportRow).join("") || emptyRow(9, "暂无举报申诉");
}
...
async function showReportDetail(id) {
  ...
  panel.innerHTML = `
    <div class="panel-head">
      <div>
        <h2>${escapeHTML(report.id ? `举报 ${report.id}` : "举报")}详情</h2>
...
async function loadTestCases() {
  ...
  $("#test-cases-table").innerHTML = state.testCases.map(testCaseRow).join("") || emptyRow(5, "暂无验收用例");
}
...
async function loadTestRuns() {
  ...
  $("#test-runs-table").innerHTML = state.testRuns.map(testRunRow).join("") || emptyRow(6, "暂无验收记录");
}
...
async function renderAdmins() {
  ...
  await Promise.all([loadAdminAccounts(), loadAdminRoles()]);
}
...
function renderIMRoomDetail(data, disputeOnly) {
  ...
  panel.innerHTML = `
    <div class="panel-head">
      <div>
        <h2>${escapeHTML(room.id ? `消息房间 ${room.id}` : "局内消息房间")}详情</h2>
        <p>${disputeOnly ? "争议证据消息" : "房间消息、文件消息和消息服务同步状态"}</p>
      </div>
    ...
    ${gameOpsTable(disputeOnly ? "争议消息" : "全部消息", ["消息编号", "发送人", "类型", "状态", "文件", "已送达", "已读", "发送时间", "操作"], messages.map(imMessageRow).join("") || emptyRow(9, "暂无消息"))}
    ${disputeOnly ? "" : gameOpsTable("文件消息", ["消息编号", "发送人", "类型", "状态", "文件", "已送达", "已读", "发送时间", "操作"], fileMessages.map(imMessageRow).join("") || emptyRow(9, "暂无文件消息"))}
  `;
...
function userRow(user) {
  return `
    <tr>
      <td>${escapeHTML(user.id ? `用户 ${user.id}` : "-")}</td>
      <td>${escapeHTML(wechatBindingText(user.openId))}</td>
      <td>${escapeHTML(adminDisplayValue(user.nickname || "-"))}</td>
...
function boundUserText(item) {
  ...
  if (item.boundWechatUserId) return `用户 ${item.boundWechatUserId}`;
  return "-";
}
...
function operationLogRow(item, index) {
  return `
    <tr>
      ...
      <td>
        <div class="row-actions">
          <button class="ghost" data-action="log-detail" data-index="${index}" type="button">查看摘要</button>
        </div>
      </td>
    </tr>
  `;
