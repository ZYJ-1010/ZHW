...
func New(authService *auth.Service, identityService identityService, gameService gameService, lbsService lbsService, imService imService) *Server {
	...
	return &Server{auth: authService, identity: identityService, games: gameService, lbs: lbsService, im: imService, reviews: reviewService, revenue: revenueService, reports: reports.NewService(revenueService), memberReports: memberreports.NewService(gameService, revenueService), membership: membership.NewService(), teams: teams.NewService(revenueService), orders: orders.NewService(), points: pointsService, redemption: redemption.NewService(pointsService), connections: connections.NewService(), profiles: profiles.NewService(), files: files.NewService(), audit: audit.NewService(), notices: notifications.NewService(), exports: exports.NewService(), admins: adminauth.NewService(), delivery: delivery.NewService(), aidata: aidata.NewService(), systemConfig: systemconfig.NewService(), reviewReplies: make(map[int64]profileReviewReply), reviewLikes: make(map[int64]map[int64]bool), mapBlindRoutes: make(map[int64]mapBlindRouteDTO), mapChallenges: make(map[int64]mapChallengeDTO), nearbyDefaultRadiusMeter: 5000}
}
...
func (s *Server) Register(mux *http.ServeMux) {
	...
	handle("POST /api/admin/reports/batch-handle", s.requireAdminPermission("report:handle", s.batchHandleReports))
...
func (s *Server) createInviteEntry(w http.ResponseWriter, r *http.Request) {
	...
	httpx.OK(w, inviteEntryResponse{
		...
		Path:       path,
		Title:      title,
		UrlLink:    s.inviteURLLink(invite.Code, invite.EntryType, path),
		Scene:      scene,
		Game:       gameInfo,
	})
}
...
func (s *Server) wechatLogin(w http.ResponseWriter, r *http.Request) {
	...
	resp.RequiresIdentityBinding = record.Status != "verified"
