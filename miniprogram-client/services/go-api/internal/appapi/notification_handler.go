...
var defaultMessageCenterConfig = map[string]interface{}{
	...
	"quickActions": []map[string]interface{}{
		{"key": "join", "label": "\u7ec4\u5c40\u52a0\u5165", "iconSrc": "https://static.haowan.net.cn/miniprogram/pages/message/assets/i53@3x.png", "tone": "blue", "bucket": "group"},
		{"key": "system", "label": "\u7cfb\u7edf\u901a\u77e5", "iconSrc": "https://static.haowan.net.cn/miniprogram/pages/message/assets/i54@3x.png", "tone": "green", "bucket": "system"},
		{"key": "achievement", "label": "\u6210\u5c31\u89e3\u9501", "iconSrc": "https://static.haowan.net.cn/miniprogram/pages/message/assets/i55@3x.png", "tone": "yellow", "bucket": "achievement"},
		{"key": "warning", "label": "\u9884\u8b66\u901a\u77e5", "iconSrc": "https://static.haowan.net.cn/miniprogram/pages/message/assets/i56@3x.png", "tone": "red", "bucket": "warning"},
...
func (s *Server) myNotifications(w http.ResponseWriter, r *http.Request) {
	...
	items := filterNotifications(s.notices.List(userID), r.URL.Query().Get("tab"), r.URL.Query().Get("type"))
	httpx.OK(w, s.notificationCenterPayload(items, r.URL.Query().Get("tab")))
}
...
func (s *Server) notificationCenterPayload(items []notifications.Notification, activeTab string) map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"activeTab":    activeTab,
		"quickActions": messageCenterQuickActions(config, items),
		"tabs":         messageCenterTabs(config, unread, trade),
		"sections":     s.messageCenterSections(config, items),
...
func messageCenterQuickActions(config map[string]interface{}, items []notifications.Notification) []map[string]interface{} {
	...
	for _, action := range actions {
		...
		item["unreadCount"] = countNotificationsByBucket(items, bucket)
		result = append(result, item)
	}
	return result
}
...
func (s *Server) messageCenterSections(config map[string]interface{}, items []notifications.Notification) []map[string]interface{} {
	sectionConfigs := sliceMapFromConfig(config, "sections")
	result := make([]map[string]interface{}, 0, len(sectionConfigs))
	for _, section := range sectionConfigs {
		item := cloneMap(section)
		bucket, _ := item["bucket"].(string)
		item["items"] = s.notificationCards(items, bucket)
		delete(item, "bucket")
		result = append(result, item)
	}
	return result
...
func filterNotifications(items []notifications.Notification, tab string, notifyType string) []notifications.Notification {
...
func (s *Server) notificationCards(items []notifications.Notification, bucket string) []map[string]interface{} {
	...
	for _, item := range items {
		...
		if actions := notificationActions(item, actionTexts); len(actions) > 0 {
...
func notificationBucket(item notifications.Notification) string {
	...
	case "game_invitation", "game_member_quit", "game_quit_result", "review_remind":
		return "group"
...
func notificationActions(item notifications.Notification, texts map[string]string) []map[string]interface{} {
	text := func(key string) string {
		return texts[key]
	}
	if item.BizType == "game_invitation" && item.Status == "unread" {
		return []map[string]interface{}{
			{"key": "accept", "text": text("accept"), "primary": true},
			{"key": "reject", "text": text("reject")},
		}
	}
	switch item.NotifyType {
	case "review_remind":
...
func countNotificationsByBucket(items []notifications.Notification, bucket string) int {
	count := 0
	for _, item := range items {
		if item.Status == "unread" && notificationBucket(item) == bucket {
			count++
		}
	}
	return count
...
func (s *Server) handleGameInvitationNotificationAction(w http.ResponseWriter, r *http.Request, notification notifications.Notification, action string, reason string) {
	...
	accept := action == "accept" || action == "confirm" || action == "join" || action == "approve"
	reject := action == "reject" || action == "decline" || action == "refuse" || action == "cancel"
	if !accept && !reject {
		httpx.Error(w, http.StatusUnprocessableEntity, httpx.CodeValidationError, "unsupported action")
		return
	}
	invitation, app, err := s.games.RespondInvitation(userID, notification.BizID, games.InvitationRespondRequest{Accept: accept, Reason: reason})
	if err != nil {
...
func (s *Server) resolveNotificationAction(notification notifications.Notification, action string) (map[string]interface{}, error) {
	...
	case "detail", "view", "open":
		return notificationDetailTarget(notification), nil
	case "process", "handle", "deliver":
		return notificationProcessTarget(notification), nil
	case "route", "navigation":
		return notificationRouteTarget(notification), nil
	case "contact", "chat":
		return notificationContactTarget(notification), nil
	case "delay":
		target := notificationDetailTarget(notification)
		target["defer"] = true
		return target, nil
	default:
...
func notificationDetailTarget(notification notifications.Notification) map[string]interface{} {
...
func notificationProcessTarget(notification notifications.Notification) map[string]interface{} {
	target := notificationTarget(notification)
	...
	case "progress_feedback_remind", "trade_warning":
		target["route"] = "pages/game/delivery/index?gameId=" + strconv.FormatInt(notification.BizID, 10)
		target["routeKey"] = "gameDelivery"
	case "report_created", "report_handled", "report_assigned", "report_closed":
		target["route"] = "pages/profile/system-management/report-record-detail/index?id=" + strconv.FormatInt(notification.BizID, 10)
		target["routeKey"] = "reportDetail"
	default:
		return notificationDetailTarget(notification)
	}
	return target
}

func notificationRouteTarget(notification notifications.Notification) map[string]interface{} {
	target := notificationTarget(notification)
