...
function buildMockCollaboration(gameId) {
  ...
  return {
    ...
    actions: {
      ...
      endConfirmRoute: `pages/game/delivery/index?gameId=${gameId}`
    }
  }
}
...
function buildMockAvailableReviews() {
  ...
  if (!mockSubmittedReviews.length) {
    return {
      reward,
  ...
  return {
    reward,
    reviewPage: mockReviewPageConfig,
    items: mockSubmittedReviews.slice(-3).map((item, index) => ({
      id: item.id || `mock-review-${index + 1}`,
      gameId: item.gameId,
      targetUserId: item.targetUserId,
      targetRole: item.targetRole || 'member',
      targetName: item.targetName || `成员 ${item.targetUserId}`,
      targetNickname: item.targetNickname || `成员${item.targetUserId}`,
...
function submitMockReview(data = {}) {
  ...
  if (!gameId || !targetUserId || score < 1 || score > 5) {
