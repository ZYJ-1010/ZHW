...
function buildPaymentPayload(data = {}) {
  ...
  return { gameId: Number(payment.gameId || 0) || 0 }
}
...
Page({
  ...
  onLoad(options = {}) {
    const gameId = Number(options.gameId || options.id || 0) || 0
    this.setData({ 'payment.gameId': gameId, paymentLoading: true, paymentError: '' })
    if (!gameId) {
      this.setData({ paymentLoading: false, paymentError: '后端未返回支付所需的局信息' })
      return
    }
    this.loadPaymentPreview(gameId)
  },

  async loadPaymentPreview(gameId) {
    try {
      const data = await gameService.getGamePaymentPreview(gameId)
      this.applyOrderPayment(data)
      this.setData({
      ...
      this.setData({ paymentLoading: false, paymentError: error.message || '后端未返回支付信息' })
    }
  },
