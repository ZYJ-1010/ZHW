...
function createGuideApplyErrors() {
  return {
    city: '',
    audience: '',
    contact: '',
...
function normalizeGuideApplyConfig(config) {
  ...
  return {
    fields,
    uploadField: normalizeUploadField(Object.assign({
    ...
    benefits: normalizeExpertPerks(rawConfig.benefits || rawConfig.perks),
    primaryText: rawConfig.primaryText || '提交领路人申请',
    helperText: rawConfig.helperText || DEFAULT_EXPERT_APPLY_CONFIG.helperText
  }
}
...
function applyGuideQualificationToConfig(config, roleInfo = {}) {
  ...
  return Object.assign({}, config, {
    requirements: (config.requirements || []).map((item) => Object.assign({}, item, {
      done: true,
      checked: true,
      met: true
    }))
...
function applyGuideFormToPage(page, form = {}) {
  if (!page || page.mode !== 'expertApplyForm' || !Array.isArray(page.fields)) {
    return page
  }

  return Object.assign({}, page, {
    fields: page.fields.map((field) => {
      if (field.type !== 'chips' || !Array.isArray(field.options)) {
        return field
      }
      const selected = Array.isArray(form[field.key]) ? form[field.key] : []

      return Object.assign({}, field, {
        options: field.options.map((option) => Object.assign({}, option, {
...
function getRoleApplyResultRoute(roleType = 'expert', status = 'approved') {
  ...
  return normalizedStatus
    ? `${ROUTES.roleStatus}?roleType=${normalizedRoleType}&status=${normalizedStatus}`
    : ''
}
...
function roleApplicationErrorMessage(error, fallback = '提交失败') {
  const message = String((error && error.message) || '').trim()

  if (/role already active|already approved/i.test(message)) {
    return '该身份已开通，无需重复申请'
  }
  if (/reapply cooldown/i.test(message)) {
    return '审核未通过后需等待 7 天再重新申请'
  }
  if (/waiting_condition/i.test(message)) {
    return '尚未满足领路人申请条件'
  }
  if (/waiting_payment/i.test(message)) {
    return '请先完成领路人资格开通要求'
  }
  if (/duplicate role application/i.test(message)) {
    return '该身份正在审核中，请勿重复提交'
  }
...
Page({
  ...
  onLoad(options = {}) {
    ...
    this.enterHomePreview(
      options.mode || 'roleComparison',
      options.single !== '0',
      options.role || options.roleType
    )
  },
  ...
  async loadExpertApplyConfig() {
    ...
    const fallbackConfig = roleType === 'guide'
      ? normalizeGuideApplyConfig({})
      : normalizeExpertApplyConfig(DEFAULT_EXPERT_APPLY_CONFIG)
  ...
  applyExpertApplyConfig(config, roleType = 'expert') {
    ...
    const normalizedConfig = applyRoleType === 'guide'
      ? normalizeGuideApplyConfig(config)
      : normalizeExpertApplyConfig(config)
    const savedDraft = readRoleApplyDraft(applyRoleType)
    ...
    if (applyRoleType === 'guide') {
      ...
      const homePreviewPages = (this.data.homePreviewPages || []).map((page) => (
        applyGuideFormToPage(applyExpertApplyConfigToPage(page, normalizedConfig), nextGuideApplyForm)
      ))
  ...
  handleExpertApplyPrimaryTap() {
    ...
    if (normalizeApplyRoleType(currentHomePreview.applyRoleType) === 'guide') {
  ...
  handleExpertApplySaveTap() {
    ...
    if (applyRoleType === 'guide') {
      const savedDraft = saveRoleApplyDraft('guide', { form: this.data.guideApplyForm })
  ...
  onGuideApplyFieldInput(event) {
    ...
    this.setData({
      [`guideApplyForm.${field}`]: event.detail.value || '',
      [`guideApplyErrors.${field}`]: ''
    })
  },
  ...
  onGuideApplyChipTap(event) {
    ...
    const fields = (this.data.currentHomePreview || {}).fields || []
    const field = fields[fieldIndex]

    if (!field || !Array.isArray(field.options) || !field.options[optionIndex]) {
      return
    }

    const options = field.options.map((item, index) => (
      index === optionIndex ? Object.assign({}, item, { active: !item.active }) : item
    ))
    const selected = options.filter((item) => item.active).map((item) => item.name).filter(Boolean)

    this.setData({
      [`currentHomePreview.fields[${fieldIndex}].options`]: options,
      [`guideApplyForm.${field.key}`]: selected,
      [`guideApplyErrors.${field.key}`]: ''
    })
  },
    ...
    wx.chooseMessageFile({
      ...
      success: (result) => {
        ...
        this.setData({
          ...
          'guideApplyErrors.uploadFiles': rawFiles.length === selectedFiles.length ? '' : `仅支持 ${acceptTypes.join('、')} 格式`
        })
      },
      fail: (error) => {
        if (!error || !String(error.errMsg || '').includes('cancel')) {
  ...
  onGuideApplyRemoveFileTap(event) {
    ...
    this.setData({ 'guideApplyForm.uploadFiles': files })
  },
  ...
  validateGuideApplyForm() {
    ...
    ;(page.fields || []).forEach((field) => {
      ...
      const empty = Array.isArray(value) ? value.length === 0 : !trimText(value)

      if (field.required && empty) {
        errors[field.key] = `请填写${field.label || '必填项'}`
      }
    })

    const guidePlan = trimText(form.guidePlan)
    if (guidePlan && (guidePlan.length < getPositiveInteger(guidePlanRule.minLength, 50) || guidePlan.length > getPositiveInteger(guidePlanRule.maxLength, 300))) {
      errors.guidePlan = `领路计划需为 ${getPositiveInteger(guidePlanRule.minLength, 50)}-${getPositiveInteger(guidePlanRule.maxLength, 300)} 个字`
    }
    if (page.uploadField && page.uploadField.required && (!form.uploadFiles || form.uploadFiles.length === 0)) {
  ...
  async handleExpertApplySubmit() {
    ...
    try {
      ...
      toast.info(roleApplicationErrorMessage(error))
    } finally {
      this.setData({
        expertApplySubmitting: false
      })
  ...
  async handleGuideApplySubmit() {
    ...
    this.setData({ expertApplySubmitting: true })
    ...
    try {
      ...
      await roleService.submitRoleApplication(buildGuideApplyPayload(this.data.guideApplyForm))
      clearRoleApplyDraft('guide')
      toast.info('领路人申请已提交，请耐心等待审核')
      setTimeout(() => {
        if (typeof wx.reLaunch === 'function') {
      ...
      this.setData({ expertApplySubmitting: false })
    }
  },
  ...
  redirectExistingRoleApply(roleInfo, roleType) {
    ...
    if (status === 'pending') {
      navigateShellRoute(`${ROUTES.roleStatus}?roleType=${normalizedRoleType}&status=pending`, {
