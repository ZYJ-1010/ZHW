...
Page({
  ...
  isRealnameVerified(user) {
    ...
    return status === 'verified' || status === 'sms_verified' || status === 'phone_verified' || status === 'approved' || status === 'passed' || user.needRealname === false || user.requiresIdentityBinding === false
  },
  ...
  async handleInvitePrimaryLogin() {
    ...
    if (this.data.isPhoneLoggingIn || this.data.isLoggingIn || this.data.hasWechatLogin) {
      return
    }
